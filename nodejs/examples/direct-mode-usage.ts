#!/usr/bin/env ts-node
/**
 * Direct Mode Usage Example for LightLLM Rust Node.js bindings (TypeScript).
 * 
 * This example demonstrates the new direct integration mode that bypasses HTTP entirely
 * for maximum performance. Perfect for TypeScript/Node.js applications that want direct
 * access to LightLLM without network overhead.
 * 
...
class DirectModeProcessor {
    private directClient: NodeLightLLMClient;
    private modelId: string;
    private token: string | null;

    constructor(modelId: string = 'llama', token: string | null = null) {
    ...
    async singleRequest(prompt: string, maxTokens: number = 100): Promise<NodeChatResponse> {
    ...
    async concurrentRequests(prompts: string[], maxTokens: number = 50): Promise<PerformanceMetrics> {
        ...
        const metrics: PerformanceMetrics = {
            totalTime: elapsed,
            averageTime: elapsed / prompts.length,
            throughput: prompts.length / (elapsed / 1000),
            successRate: (responses.length / prompts.length) * 100
        };
        
        console.log(`✅ Direct concurrent processing completed: ${responses.length} responses in ${elapsed.toFixed(1)}ms`);
        console.log(`   Average per request: ${metrics.averageTime.toFixed(1)}ms`);
...
class ModeComparison {
    private directClient: NodeLightLLMClient;
    private httpClient: NodeLightLLMClient;
    ...
    async testDirectMode(prompt: string, maxTokens: number = 50): Promise<NodeChatResponse> {
        const messages = [new NodeMessage('user', prompt)];
    ...
    async testHttpMode(prompt: string, maxTokens: number = 50): Promise<NodeChatResponse> {
        const messages = [new NodeMessage('user', prompt)];
...
async function main(): Promise<void> {
    console.log('🚀 LightLLM Rust Node.js Bindings - Direct Mode Usage (TypeScript)');
    ...
    try {
        ...
        const metrics = await processor.concurrentRequests(prompts, 30);
        console.log(`✅ Concurrent direct processing successful: ${prompts.length} responses`);
